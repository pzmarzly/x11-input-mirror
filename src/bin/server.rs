...
fn handler_thread(conn: &mut TcpStream, max_ping: Duration, password: &str) {
    ...
    let addr = conn
        .peer_addr()
        .expect("Client disconnected - peer_addr failed");
    conn.set_nodelay(true)
        .unwrap_or_else(|_| panic!("Client {} disconnected - set_nodelay failed", addr));
    conn.set_read_timeout(Some(max_ping))
        .unwrap_or_else(|_| panic!("Client {} disconnected - set_read_timeout failed", addr));
    conn.set_write_timeout(Some(max_ping))
        .unwrap_or_else(|_| panic!("Client {} disconnected - set_write_timeout failed", addr));
    ...
    conn.read_exact(&mut buf)
        .unwrap_or_else(|_| panic!("Client {} disconnected - handshake3 failed", addr));
    if buf[0] != should_be_encrypted_num {
        panic!(
            "Client with wrong SecurityConfig tried to connect from {}",
            addr
        );
    }
    ...
    conn.write_all(&buf)
        .unwrap_or_else(|_| panic!("Client {} disconnected - handshake4 failed", addr));
    ...
    let mut buf = [0u8; 14];
    conn.read_exact(&mut buf)
        .unwrap_or_else(|_| panic!("Client {} disconnected - handshake5 failed", addr));
    chacha.xor(&mut buf);
    ...
    // handshake6
    conn.write_all(b"\x01")
        .unwrap_or_else(|_| panic!("Client {} disconnected - handshake6 failed", addr));
    ...
    loop {
        let mut buf = [0u8; 16];
        conn.read_exact(&mut buf)
            .unwrap_or_else(|_| panic!("Client {} disconnected", addr));
